...
export async function POST(req: NextRequest) {
  try {
    ...
    const hasValidKey = Boolean(apiKey && apiKey.startsWith('sk-ant-') && !apiKey.includes('placeholder'));

    if (hasValidKey) {
      // Real Anthropic Claude Tool Calling Execution
      ...
      const initialResponse = await anthropic.messages.create({
        ...
        max_tokens: 1024,
        system: `You are MerchantOS Decision Agent, an AI decision co-pilot for small business owners.
...
NEVER say "approved" or "you qualify" for loans (use "appears financially prepared for additional working-capital financing, subject to lender underwriting").`,
      ...
      // Deterministic Engine Tool Orchestration (Fallback when ANTHROPIC_API_KEY is not set)
